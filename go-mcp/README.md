...
[MCP](https://modelcontextprotocol.io) server exposing the Radiorecord SDK as
two agent tools — `radiorecord_list` and `radiorecord_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/radiorecord-mcp)
make build

# 2. Provide credentials via the environment
export RADIORECORD_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user radiorecord \
  -- /absolute/path/to/radiorecord-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./radiorecord-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// radiorecord_list: first page of records
{ "entity": "chart" }
{ "entity": "chart", "query": { } }

// radiorecord_load: one record by id
{ "entity": "chart", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/radiorecord-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user radiorecord \
     -- "$PWD"/dist/*/radiorecord-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `radiorecord_list` and `radiorecord_load` tools now appear
   in new sessions. Ask the agent to *"list chart using radiorecord"*
   and it calls `radiorecord_list` with `{"entity":"chart"}`.
...
```sh
export RADIORECORD_APIKEY=sk_live_xxx            # API key
export RADIORECORD_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `radiorecord_list` tool
...
### Call the `radiorecord_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `radiorecord_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `radiorecord_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RADIORECORD_APIKEY` | API key sent with every request. |
| `RADIORECORD_BASE` | Optional override of the API base URL. |
...
```sh
./radiorecord-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"radiorecord_load","arguments":{"entity":"chart","query":{"id":1}}}}'
```
