...
class RadiorecordMakeError
{
    public static function call(?RadiorecordContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RadiorecordContext([], null);
        }
        $op = $ctx->op ?? new RadiorecordOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RadiorecordResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RadiorecordError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new RadiorecordError('', $msg, $ctx);
        ...
        if ($err instanceof RadiorecordError) {
