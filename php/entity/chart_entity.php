...
class ChartEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ChartEntity($this->_client, $opts);
    }

    /**
     * @param Chart|array $args Chart data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RadiorecordHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Chart|array The current Chart data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Chart fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RadiorecordHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Chart fields).
     */
    ...
    /**
     * List Chart items matching the given filter.
     *
     * @param ChartListMatch|array|null $reqmatch Match filter (any subset
     *   of Chart fields) as an assoc-array; ChartListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Chart[]|array A list of Chart items as assoc-arrays at
     *   the SDK boundary; throws RadiorecordError on failure (item-5 convention).
     */
