...
//     (\`new RadiorecordSDK(...)\` / \`RadiorecordSDK.test(...)\` become
//     \`RadiorecordSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"chart":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RadiorecordSDK(...)\` and any
// \`RadiorecordSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RadiorecordSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RadiorecordSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
