...
local ChartEntity = {}
ChartEntity.__index = ChartEntity


function ChartEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ChartEntity)
  self._name = "chart"
  self._client = client
...
function ChartEntity:get_name()
  return self._name
end


function ChartEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ChartEntity.new(self._client, opts)
end
...
function ChartEntity:mark_deleted()
  self._deleted = true
end


function ChartEntity:deleted()
  return true == self._deleted
end


function ChartEntity:data_set(args)
  if args ~= nil then
...
function ChartEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ChartEntity:match_set(args)
  if args ~= nil then
...
function ChartEntity:match_get()
...
function ChartEntity:stream(action, args, callopts)
...
function ChartEntity:_run_op(ctx, post_done)
  local utility = self._utility
