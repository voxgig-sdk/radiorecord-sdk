...
class TestChartDirect:

    def test_should_direct_list_chart(self):
        setup = _chart_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-chart", "live" if setup["live"] else "unit")
        if _skip:
...
def _chart_direct_setup(mockres):
