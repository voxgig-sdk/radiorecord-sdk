...
from radiorecord_sdk.radiorecord_types import (
    Chart,
    ChartListMatch,
)
...
class ChartEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ChartEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Chart]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Chart().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
