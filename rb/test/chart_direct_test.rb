...
class ChartDirectTest < Minitest::Test
  def test_direct_list_chart
    setup = chart_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-chart", setup[:live] ? "live" : "unit")
    if _should_skip
...
def chart_direct_setup(mockres)
  ...
  if live
    ...
    client = RadiorecordSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
