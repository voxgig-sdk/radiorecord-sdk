...
require_relative 'Radiorecord_types'
...
class RadiorecordSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RadiorecordHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RadiorecordHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RadiorecordFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RadiorecordUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RadiorecordHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RadiorecordHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RadiorecordHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RadiorecordHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RadiorecordError.new(
        "graphql_error", "RadiorecordSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RadiorecordSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
