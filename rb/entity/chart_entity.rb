...
class ChartEntity
  ...
  def make
    opts = @_entopts.dup
    ChartEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RadiorecordHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Chart, Hash] the current Chart data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RadiorecordHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Chart fields)
  def match_get
  ...
  #
  # @param reqmatch [ChartListMatch, Hash, nil] match filter (any subset of
  #   Chart fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Chart>, Array] the matching Chart items; raises RadiorecordError on failure
