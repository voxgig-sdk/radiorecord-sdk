...
class FtCtrl extends RadiorecordControl
{
...
class FtClient
{
    ...
    private RadiorecordUtility $utility;

    public function __construct(RadiorecordUtility $utility, array $options)
    {
    ...
    public function get_utility(): RadiorecordUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RadiorecordContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RadiorecordContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RadiorecordContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RadiorecordConfig::make_feature($name);
    ...
    private static function buildUrl(RadiorecordSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RadiorecordError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RadiorecordResponse($fetched);
        }
    ...
    private function fail(RadiorecordContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RadiorecordContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
