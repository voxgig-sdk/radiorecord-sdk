...
class RadiorecordMakeRequest
{
    public static function call(RadiorecordContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new RadiorecordResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new RadiorecordResponse($fetched);
        } else {
