...
module RadiorecordUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RadiorecordResponse.new({})
    ...
    if fetch_err
      ...
      response = RadiorecordResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RadiorecordResponse.new(fetched)
    else
