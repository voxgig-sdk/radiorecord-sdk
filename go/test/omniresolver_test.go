...
import (
	...
	omni "github.com/voxgig-sdk/radiorecord-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RadiorecordError); is {
				out := map[string]any{
					"name":    "RadiorecordError",
...
func omniCtx(arg any, client *sdk.RadiorecordSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RadiorecordSDK); is {
			client = live
