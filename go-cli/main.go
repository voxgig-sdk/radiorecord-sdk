// Command Radiorecord-cli is a CLI + REPL for the Radiorecord SDK,
...
const prompt = "radiorecord"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "chart"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      RADIORECORD_APIKEY (api key), RADIORECORD_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: RADIORECORD_APIKEY carries the API key and
	// RADIORECORD_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("RADIORECORD_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("RADIORECORD_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRadiorecordSDK(opts)
