...
boru-driven command-line client **and** interactive REPL for the Radiorecord
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/radiorecord-cli)
make build

# 2. See usage (words, entities, env vars)
./radiorecord-cli --help

# 3. Provide credentials once, via the environment
export RADIORECORD_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./radiorecord-cli list chart

# 5. Override the API base URL for a single call
RADIORECORD_BASE=https://api.example.com ./radiorecord-cli list chart

# 6. No arguments -> interactive REPL
./radiorecord-cli
radiorecord> list chart
radiorecord> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/radiorecord-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/radiorecord-cli list chart
   ```
...
Run with no arguments to open a REPL (prompt `radiorecord>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `chart`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/radiorecord-cli`. |
...
(`./radiorecord-cli <expr>`) and interactively (the REPL), and expressions compose the
